...
namespace TypicalTool {
	...
	Tstring TimeMeasureToString(TimeMeasure _Tm) {
		switch (_Tm) {
		case TimeMeasure::ns: {
			return TEXT("ns");
		}
...
#define TimeToStr TimeMeasureToString
	...
	Tstring TimeAutoToStr(int64_t _Time, TimeMeasure _Tm, bool _bShowMeasure = true, int32_t _FormatPlaceholder = 1, const Tstring& _TimeIntervalStr = TEXT(" "),
		const Tstring& _NumberLeftIntervalStr = TEXT(""), const Tstring& _NumberRightIntervalStr = TEXT("")) {
	...
	private:
		...
		void Time_IsValid_RunTime(int32_t _Number, const Tstring& _FunctionName)
		{
			if (this->IsSaveAllTimePoint) {
	...
	public:
		...
		{
			if (IsShowLog) {
				LogRelease(LogLevelFlags::Warning, Printf(TEXT("休眠: [%s]%s"), _Number, TimeMeasureToString<Target>()));
			}
		...
		{
			if (IsShowLog) {
				LogRelease(LogLevelFlags::Warning, Printf(TEXT("等待: [%s]%s"), _Number, TimeMeasureToString<Target>()));
			}
		...
		template<typename T = bool>
		static void FormatTime(Tstring& text, const Tstring& timeFormat = TEXT("%Y-%m-%d %H:%M:%S"),
			const Tstring& textLeftFormat = TEXT("["), const Tstring& textRigthFormat = TEXT("]"))
		{
		...
		template<typename T = bool>
		static Tstring GetFormatTime(const Tstring& timeFormat = TEXT("%Y-%m-%d %H:%M:%S"),
			const Tstring& textLeftFormat = TEXT("["), const Tstring& textRigthFormat = TEXT("]"))
		{
			std::chrono::system_clock::time_point now = std::chrono::system_clock::now();;
			// 获取当前时间点（自epoch以来的时间）
