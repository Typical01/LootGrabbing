...
#ifndef UTF8
		template<typename... Args>
		Printf(const std::string& format, Args... args) 
		{
			std::vector<std::string> placeholders = { format };
			this->FormatStr = stow(format_impl(placeholders, args...));
		}
#endif
#endif
...
#ifndef UTF8
		// 基础模板：处理最后一个参数
		static std::string format_impl(std::vector<std::string>& placeholders) { return placeholders[0]; }
		// 递归模板：处理其他参数
		template<typename T, typename... Args>
		static std::string format_impl(std::vector<std::string>& placeholders, T first, Args... args)
		{
			std::string formatted_string = placeholders[0];
			size_t pos = formatted_string.find(TEXT("%s")); // 查找第一个占位符
...
#ifdef UNICODE
#ifndef UTF8
		void operator()(const std::string& _Text, LogMessage(*_Lm)() = &Lnf, const int& _MessageKey = -1)
		{
			if (!bShowLog) { return; }

			if (Debug) { // Debug
				Logs_ustr(stow(_Text), _Lm(), _MessageKey);
			}
			else { // Release
				if (this->Release) {
					Logs_ustr(stow(_Text), _Lm(), _MessageKey);
